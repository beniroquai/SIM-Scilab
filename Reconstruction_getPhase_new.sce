...
mag = 20;     // magnification of Lens (@ TL=400mm)/fMO=20mm
...
for j = 1:ntheta
    ...
    for i = 1:nphi
        ...
        phi_est((j-1)*3+i) = atan2(imag(fourier(a(2),a(1))), real(fourier(a(2),a(1))))


        if phi_est((j-1)*3+i) <= 0 then 
            phi_est((j-1)*3+i)= phi_est((j-1)*3+i) + %pi;
        end
    ...
    while(rot_angle<0) rot_angle = rot_angle +%pi;
    end
...
for i=1:ntheta
    ...
    for k = 1:(n)
        ...
        y_pos(k) = round(tan(theta(j))*x_pos(k))+1;
        disp(y_pos(k))
        if (y_pos(k) < 1) break;
    ...
    max_freq = max_freq(zero_ordermax:50)
    [u, v] = max(max_freq)
    frequency_est = frequency_est+v+zero_ordermax;
end

//Assume, Frequency of Pattern is always the same!
frequency_est = 1/(frequency_est/nphi);       // 2*Pixelperiode equals 1/f
...
for i=1:nphi
    ...
    amplitude = .7*(max(value_sub)-min(value_sub))/2
    //    frequency = 2*frequency_est//.09//2*%pi*0.1   //max(fourier)
    frequency = 1/(fc_est(i));
    //frequency = 1/9;
    phase = 0// phi(i);
